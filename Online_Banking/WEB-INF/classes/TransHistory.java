	...
	try
		{
     		...
     		if(session != null)
			{
				...
				if(email.equals(sess_email) && password.equals(sess_password))
				{
					...
					ArrayList dt = obj.viewTransHistory(email,password);
					
					out.println("<table><tr><th>ACCOUNT ID</th><th>CURRENT BALANCE</th><th>TRANSATION</th><th>TRANSACTION BALANCE</th><th>TRANSACTION DATE</th><th>ACCOUNT DATE</th></tr>");
					out.println("<h1>HERE IS YOUR TRANSACTION HISTORY!!</h1>");
					for (int i = 0; i < dt.size(); i++) 
					{
						out.println("<tr><td>"+((Details)dt.get(0)).getEmail()+"</td><td>"+((Details)dt.get(i)).getBalance()+"</td><td>"+((Details)dt.get(i)).getTransaction()+"</td><td>"+((Details)dt.get(i)).getTransBalance()+"</td><td>"+((Details)dt.get(i)).getTransDate()+"</td><td>"+((Details)dt.get(i)).getAccDate()+"</td>");
					}
